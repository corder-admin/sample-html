...
const median = calculateMedian(prices); // → 10000円
```
...
**IQR 法とは**:
データを小さい順に並べて 4 等分した時、真ん中 50%のデータの幅を使って外れ値を見つける方法です。平均値と違い、極端な値に影響されにくい特徴があります。

**具体例で理解する**:

```javascript
// 例: パネル加工費の単価データ（円）
const prices = [8000, 8500, 9000, 9500, 10000, 10500, 11000, 50000];
// 8番目のデータ（50000円）が異常に高いかどうかを判定したい

// ステップ1: データを小さい順に並べる（既にソート済み）

// ステップ2: 25%地点（Q1）と75%地点（Q3）を求める
const q1Index = Math.floor(prices.length * 0.25); // インデックス2
const q3Index = Math.floor(prices.length * 0.75); // インデックス6
const q1 = prices[q1Index]; // 9000円
const q3 = prices[q3Index]; // 11000円

// ステップ3: IQR（四分位範囲）を計算
const iqr = q3 - q1; // 11000 - 9000 = 2000円
```

**外れ値の判定基準**:

```javascript
// 通常の外れ値（Category B: 要確認）
const upperFence = q3 + 1.5 * iqr; // 11000 + 1.5 × 2000 = 14000円
// → 14000円を超えるデータは「要確認」

// 極端な外れ値（Category A: 自動除外）
const extremeUpperFence = q3 + 3.0 * iqr; // 11000 + 3.0 × 2000 = 17000円
// → 17000円を超えるデータは「自動除外」

// この例では 50000円 > 17000円 なので「極端な外れ値（Category A）」と判定
```

**視覚的イメージ**:

```text
データ分布:  [8000]──[9000]────[中央値]────[11000]──[50000]
               ↑      ↑                    ↑         ↑
              最小    Q1                   Q3      異常値

正常範囲:            [======== IQR ========]
要確認範囲:    ←1.5×IQR→              ←1.5×IQR→
除外範囲:  ←3.0×IQR→                          ←3.0×IQR→
```
...
// 例: 同じ項目の単価データ
const itemPrices = [1000, 1100, 1200, 5000];
const cv = calculateCV(itemPrices); // → 約85% (バラツキあり)
```
...
```javascript
// 個別の単価が標準単価（中央値）からどれだけ離れているかを%で計算
function calculateDeviationRate(price, medianPrice) {
  const deviation = Math.abs(price - medianPrice);
  const deviationRate = (deviation / medianPrice) * 100;
  return deviationRate;
}

// 例: パネル加工費の標準単価が10,000円の時
const standardPrice = 10000;
const actualPrice = 50000;
const rate = calculateDeviationRate(actualPrice, standardPrice);
// → 400% (4倍の乖離)
```
